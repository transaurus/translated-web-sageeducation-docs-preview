...
:::warning
このチュートリアルはコミュニティによる貢献であり、Sage Open WebUIチームによるサポート対象外です。特定のユースケース向けにSage Open WebUIをカスタマイズする方法のデモンストレーションとして提供されています。貢献したいですか？ コントリビューションガイドをご確認ください。
:::

> [!WARNING]  
> このドキュメントは現在のバージョン（0.5.11）に基づいて作成されており、継続的に更新されています。

# sage-open-webui 内部SQLiteデータベース

sage-open-webuiにおいて、SQLiteデータベースはユーザー管理、チャット履歴、ファイルストレージ、その他様々なコア機能を支える中核的な役割を果たしています。この構造を理解することは、プロジェクトへの貢献やメンテナンスを効果的に行う上で不可欠です。

## 内部SQLiteの場所

SQLiteデータベースは `root` -> `data` -> `webui.db` に配置されています
...
コンテナ内で動作しているsage-open-webuiのSQLiteデータベースをローカルマシンにコピーしたい場合、以下のコマンドを使用できます：

```bash
docker cp sage-open-webui:/app/backend/data/webui.db ./webui.db
```

あるいは、コンテナ内から直接データベースにアクセスする場合は：
...
以下はsage-open-webuiのSQLiteデータベースに含まれるテーブルの完全なリストです。テーブルはアルファベット順に番号付きで記載されています。
...
注：sage-open-webuiのSQLiteデータベースにはコア機能に関連しない以下の2つのテーブルが存在しますが、これらはリストから除外しています：

- Alembicバージョンテーブル
- マイグレーション履歴テーブル

すべてのテーブルを把握したところで、各テーブルの構造を理解していきましょう。
...
フォルダーテーブルについて知っておくべき点：

- フォルダーはネスト可能（parent_idで参照）
- ルートフォルダーはparent_idがnull
- フォルダー名は同一親フォルダー内で一意である必要あり
...
- `type`は["filter", "action"]のいずれかのみ許可
...
テーブル間の関係を視覚化するために、以下にMermaidで生成したエンティティ関係図（ERD）を参照してください。
